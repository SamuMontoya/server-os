...
export async function syncDriveFolder(folderId: string): Promise<DriveSyncResult | null> {
  ...
  // Solo cuentan como "al día" las filas que además tienen vector: si el hash
  // coincide pero embedding/embedding_local quedó null (proveedor apagado en
  // un sync anterior, fallo silencioso de embed, etc.), se re-embebe igual —
  // si no, un hash sin cambios lo dejaría huérfano de vector para siempre.
  const known = new Map<string, string>();
  {
    ...
    for (let from = 0; ; from += PAGE) {
      const { data, error } = await supabase
        .from("drive_docs")
        .select(`drive_file_id,content_hash,${EMB.col}`)
      ...
      for (const r of data as Record<string, unknown>[]) {
        if (r[EMB.col] == null) continue; // sin vector: tratarlo como pendiente
        known.set(r.drive_file_id as string, r.content_hash as string);
      }
